...
export const BusinessSummarySection: React.FC<BusinessSummarySectionProps> = ({
  ...
  const overallMetrics = [
    {
      ...
      value: formatCurrency(revenue?.customerSales ?? revenue?.grossSales ?? accounting?.grossSales ?? revenue?.totalRevenue),
      icon: DollarSign,
      colorBg: "bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400",
    },
    {
      title: "Total Product Selling",
      value: formatCurrency(revenue?.productSelling ?? revenue?.grossSales ?? accounting?.grossSales),
      icon: ShoppingCart,
      colorBg: "bg-cyan-50 dark:bg-cyan-950/60 text-cyan-600 dark:text-cyan-400",
    },
    {
      title: "Total Product Cost",
      value: formatCurrency(revenue?.productCost ?? accounting?.totalProductCost),
      icon: Package,
      colorBg: "bg-violet-50 dark:bg-violet-950/60 text-violet-600 dark:text-violet-400",
    },
    {
      title: "Total Courier Cost",
      value: formatCurrency(revenue?.courierCost ?? accounting?.courierCost ?? revenue?.courierServiceCost),
      icon: Truck,
      colorBg: "bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400",
    },
    {
      title: "Total Net Profit",
      value: formatCurrency(revenue?.netProfit ?? accounting?.netProfit),
      icon: TrendingUp,
  ...
  const todayMetrics = [
    {
      title: "Today's Customer Sales",
      value: formatCurrency(revenue?.todayCustomerSales ?? revenue?.todaySales ?? accounting?.todaySales ?? revenue?.todayRevenue),
      icon: Calendar,
      colorBg: "bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400",
    },
    {
      title: "Today's Product Selling",
      value: formatCurrency(revenue?.todayProductSelling ?? revenue?.todaySales ?? accounting?.todaySales),
      icon: ShoppingCart,
    ...
    },
    {
      ...
      value: formatCurrency(revenue?.todayNetProfit ?? revenue?.todayProfit ?? accounting?.todayProfit),
      icon: TrendingUp,
      colorBg: "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400",
    },
    {
      title: "Today's Quantity Sold",
      value: formatNumber(revenue?.todayQuantitySold ?? revenue?.todayQuantity ?? products?.todayUnits ?? accounting?.todayQuantity),
      icon: ShoppingCart,
      colorBg: "bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Overall Business Summary Section */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Lifetime business performance ledger from backend response
              </p>
            </div>
          </div>
          <span className="px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wider rounded-lg bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300 border border-indigo-200/60">
            Overall Ledger
          </span>
        </div>
      ...
      {/* Today's Business Summary Section */}
      <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Daily real-time performance ledger from backend response
              </p>
            </div>
          </div>
